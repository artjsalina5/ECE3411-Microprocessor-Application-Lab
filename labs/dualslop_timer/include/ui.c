...
static void cmd_debug_tca0(const char *params);
...
static const command_t commands[] = {
    ...
    {"DEBUG RTC", cmd_debug_info, "DEBUG          - Show RTC debug info"},
    {"DEBUG TCA0", cmd_debug_tca0, "DEBUG TCA0       - Show TCA0 debug info"},
...
void ui_show_welcome(void) {
  printf("\nArturo's Operating System Debugger\n");
...
void ui_display_time(void) {
  ...
  if (alarm_set) {
    printf("Alarm Set: %02d:%02d:%02d", alarm_time.hours, alarm_time.minutes,
           alarm_time.seconds);
    if (alarm_triggered) {
      printf(" [TRIGGERED!]");
    }
...
static void cmd_set_alarm(const char *params) {
  ...
  if (ui_parse_time(params, &new_alarm)) {
    ...
    sei();
    printf("Alarm set to %02d:%02d:%02d\n", new_alarm.hours, new_alarm.minutes,
           new_alarm.seconds);
  } else {
    printf("Invalid time format. Use HH:MM:SS\n");
  }
...
static void cmd_debug_info(const char *params) {
  ...
  printf("  Clock source: %s\n",
         ...
         : (RTC.CLKSEL == RTC_CLKSEL_XOSC32K_gc) ? "External 32kHz"
                                                 : "Other");
}
