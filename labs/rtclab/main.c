...
// Global variables for time keeping (accessed by UI module)
...
ISR(TCA0_OVF_vect) {
  ...
  if (alarm_triggered) {
    ...
    led_blink_counter = 0;
    PORTD.OUTCLR = PIN3_bm; // Turn off LEDs when no alarm
  }
...
ISR(RTC_CNT_vect) {
  ...
  // With internal 32kHz oscillator and PER=32768, we get exactly 1 second interrupts
  // Increment seconds directly
