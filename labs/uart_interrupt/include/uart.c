...
/* Interrupt-driven circular buffers */
static volatile uart_buffer_t uart_rx_buffer = {{0}, 0, 0, 0};
static volatile uart_buffer_t uart_tx_buffer = {{0}, 0, 0, 0};
...
static bool buffer_put(volatile uart_buffer_t *buf, char data) {
  // ring buffer
  if (buf->count < UART_BUFFER_SIZE) { // if the buffer is not full
    buf->buffer[buf->head] = data;     // Give data to the buffer
    buf->head = (buf->head + 1) %
                UART_BUFFER_SIZE; // this wraps arround in powers of two
    buf->count++;                 // Master count of data
    return true;
  }
  return false; // Buffer full
}

static bool buffer_get(volatile uart_buffer_t *buf, char *data) {
  if (buf->count > 0) {
...
uint8_t uart_tx_free_space(void) {
  return UART_BUFFER_SIZE - uart_tx_buffer.count;
}

uint8_t uart_rx_available(void) { return uart_rx_buffer.count; }
