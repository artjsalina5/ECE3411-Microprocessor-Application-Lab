...
void ui_display_time(void) {
  ...
  if (countdown_set) {
    aos_printf("Countdown Set: %02d:%02d", countdown_time.minutes,
  ...
  */
  ...
  // Add status line
  if (countdown_finished) {
    aos_send("Status: Countdown Complete!!!!\r\n");
  } else if (countdown_set) {
    aos_send("Status: Counting Down...\r\n");
  } else {
...
static void collect_uart_input(void) {
  ...
  while (uart_receive_char(&ch)) {
    // Handle single character commands for countdown control
    if (ch == 'p' || ch == 'P') {
      uart_send_char(ch); // Echo
      aos_send("\r\n");
      extern void pause_countdown(void);
      pause_countdown();
      aos_send("AOS> ");
      continue;
    }
    ...
    if (ch == 'r' || ch == 'R') {
      uart_send_char(ch); // Echo
      aos_send("\r\n");
      extern void resume_countdown(void);
      resume_countdown();
      aos_send("AOS> ");
      continue;
    }

    // Echo character for other inputs
    uart_send_char(ch);
