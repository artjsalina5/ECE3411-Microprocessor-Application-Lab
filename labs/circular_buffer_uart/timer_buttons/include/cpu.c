 ...
 * @{
 *
 * This module provides functions for configuring the CPU clock and external
 * crystal oscillator for AVR128DB48 microcontroller. It includes initialization
 * routines for both internal and external clock sources with proper protected
 * register writes.
 */
 ...
 * clock control registers that are protected by the Configuration Change
 * Protection.
 *
 * @note This function should be called before switching to external clock
 * source as the main system clock.
 *
 * @return void
 */
void External_Crystal_init(void) {
  ...
  _protected_write_io((void *)&CLKCTRL.XOSCHFCTRLA, CCP_IOREG_gc, buffer);
