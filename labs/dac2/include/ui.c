...
//================================
extern volatile uint16_t timer_reload_value;
extern volatile uint16_t samples_per_cycle;
...
static void cmd_countdown_status(const char *params);

// Waveform generator commands
static void cmd_frequency(const char *params);
static void cmd_amplitude(const char *params);
static void cmd_waveform_status(const char *params);
...
static const command_t commands[] = {
    ...
    // Waveform Generator Commands
    {"FREQ", cmd_frequency,
     "FREQ <10-1000>          - Set sine wave frequency (Hz)"},
    {"AMP", cmd_amplitude,
     "AMP <10-100>            - Set amplitude percentage"},
    {"WAVE", cmd_waveform_status,
     "WAVE                    - Show waveform status"},
    ...
    {"COUNTDOWN", cmd_countdown_status,
     "COUNTDOWN               - Show countdown status"},
...
static void collect_uart_input(void) {
  ...
  while (uart_receive_char(&ch)) {
    // Echo character for command line input
    uart_send_char(ch);
...
static void cmd_set_alarm(const char *params) {
  (void)params;
  aos_send("Alarm functionality has been replaced with countdown timer.\r\n");
  aos_send("Press button B5 to start countdown from current time.\r\n");
  aos_send("Use 'p' to pause, 'r' to resume, 'COUNTDOWN' for status.\r\n\r\n");
}
...
static void cmd_stop_alarm(const char *params) {
  (void)params;
  aos_send("Alarm functionality has been replaced with countdown timer.\r\n");
  aos_send("To stop countdown blinking, restart the system or wait 5 "
           "seconds.\r\n\r\n");
}

//*****************************************************************************
// Frequency and Amplitude DAC Output Control
// ****************************************************************************
static void cmd_change_buzzer_amplitude(const char *params) {
  uint8_t new_buzzer_amplitude;
  // changes global buzzer_amplitude from 10% to 100% of VREF (3.3V)
  VREF.DAC0REF = VREF_REFSEL_VDD_gc;
}

static void cmd_change_buzzer_frequency(const char *params) {
  uint16_t new_buzzer_frequency;
  // changes buzzer_frequency global variable from 10hz-1000hz
}
static void cmd_countdown_status(const char *params) {
  (void)params;
  extern volatile bool countdown_set, countdown_finished, countdown_paused;
  extern volatile rtc_time_t countdown_time;

  aos_send("=== COUNTDOWN STATUS ===\r\n");

  if (!countdown_set) {
    aos_send("Status: INACTIVE\r\n");
    aos_send(
        "Action: Press button B5 to start countdown from current time\r\n");
  } else if (countdown_finished) {
    aos_send("Status: FINISHED (00:00)\r\n");
    aos_send("Action: Press button B5 to start new countdown\r\n");
  } else {
    aos_printf("Status: %02d:%02d remaining", countdown_time.minutes,
               countdown_time.seconds);
    if (countdown_paused) {
      aos_send(" - PAUSED\r\n");
      aos_send("Action: Press 'r' to resume countdown\r\n");
    } else {
      aos_send(" - COUNTING DOWN\r\n");
      aos_send("Action: Press 'p' to pause countdown\r\n");
    }
  }

  aos_send("Commands: 'p'=pause, 'r'=resume, B5=start new\r\n");
  aos_send("\r\n");
}

//================================
// Waveform Generator Command Implementations
//================================

// Calculate and update timer frequency based on buzzer_frequency
static void update_timer_frequency_and_apply(void) {
  // Calculate timer reload value
  // Sample rate = frequency * samples_per_cycle
  // TCA1 frequency = 2MHz (16MHz / 8 prescaler)
  // Timer period = 2MHz / (reload + 1)
  // We want: 2MHz / (reload + 1) = frequency * samples_per_cycle
  // So: reload = (2MHz / (frequency * samples_per_cycle)) - 1

  uint32_t target_sample_rate = (uint32_t)buzzer_frequency * samples_per_cycle;
  uint32_t timer_freq = 2000000UL; // 16MHz / 8 prescaler

  if (target_sample_rate > 0) {
    timer_reload_value = (timer_freq / target_sample_rate) - 1;

    // Ensure reasonable bounds for TCA1 timer (16-bit)
    if (timer_reload_value < 19)     // Minimum for 1000Hz with 100 samples at 2MHz
      timer_reload_value = 19;
    if (timer_reload_value > 65535)
      timer_reload_value = 65535;
  }

  // Apply the new period to the timer safely
  uint8_t old_sreg = SREG;
  cli();                                       // Disable interrupts briefly
  TCA1.SINGLE.CTRLA &= ~TCA_SINGLE_ENABLE_bm; // Disable TCA1
  TCA1.SINGLE.CNT = 0;                        // Reset counter to prevent glitches
  TCA1.SINGLE.PER = timer_reload_value;       // Set new period
  TCA1.SINGLE.INTFLAGS = TCA_SINGLE_OVF_bm;   // Clear any pending interrupts
  TCA1.SINGLE.CTRLA |= TCA_SINGLE_ENABLE_bm;  // Re-enable TCA1
...
static void cmd_frequency(const char *params) {
  ...
  uint16_t new_freq = atoi(params);
  if (new_freq >= 10 && new_freq <= 1000) {  // Back to 1000 Hz max
    buzzer_frequency = new_freq;

    // Calculate and apply new timer frequency in one step
    update_timer_frequency_and_apply();

    aos_printf("Frequency set to %d Hz\r\n", buzzer_frequency);
  } else {
    aos_send("Error: Frequency must be between 10 and 1000 Hz\r\n");
  }
}
...
static void cmd_amplitude(const char *params) {
  if (!params || strlen(params) == 0) {
    ...
    aos_send("Usage: AMP <10-100>\r\n");
    return;
  }

  uint16_t new_amp = atoi(params);
  if (new_amp >= 10 && new_amp <= 100) {
    buzzer_amplitude = (uint8_t)new_amp;
    aos_printf("Amplitude set to %d%%\r\n", buzzer_amplitude);
  } else {
    aos_send("Error: Amplitude must be between 10 and 100 percent\r\n");
  }
}
...
static void cmd_waveform_status(const char *params) {
  ...
  aos_printf("Frequency: %d Hz\r\n", buzzer_frequency);
  aos_printf("Amplitude: %d%% of Vref\r\n", buzzer_amplitude);
  aos_printf("Waveform: Sine wave (%d samples)\r\n", samples_per_cycle);
  aos_printf("Sample Rate: %lu Hz\r\n",
             (unsigned long)(buzzer_frequency * samples_per_cycle));
  aos_printf("TCA1 Period Value: %d\r\n", timer_reload_value);
  aos_send("DAC Output: Enabled\r\n");
  aos_send("Commands: FREQ <10-500>, AMP <10-100>\r\n");
  aos_send("\r\n");
}
