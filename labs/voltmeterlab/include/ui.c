...
//================================
// External Variables from main.c (Waveform Generator)
//================================
...
static volatile bool waveform_active = true;  // Whether to show interactive prompts
...
static const command_t commands[] = {
    ...
    {"FREQ", cmd_freq,
     "FREQ <10-1000>          - Set frequency in Hz"},
    {"AMPL", cmd_ampl,
     "AMPL <10-100>           - Set amplitude as percentage"},
    ...
    {"ADCLAB", cmd_run_adclab, "ADCLAB                  - Display waveform generator status"},
...
void ui_show_welcome(void) {
  aos_send("\r\n");
  aos_send("+-----------------------------------------------------------+\r\n");
  aos_send("|          PROGRAMMABLE WAVEFORM GENERATOR (ADCLab)         |\r\n");
  aos_send("|                          BOOTED!                          |\r\n");
  aos_send("+-----------------------------------------------------------+\r\n");
...
void ui_status_printing(void) {
  ...
  case 0:
    ...
    break;
  case 1:
    aos_printf("New Frequency Value %lu press Enter to confirm: ", ui_temp_freq);
    break;
  case 2:
...
static void collect_uart_input(void) {
  ...
  while (uart_receive_char(&ch)) {
    // If in waveform interactive mode, handle character input differently
    if (waveform_active && ui_mode != 0) {
      // In frequency or amplitude entry mode
      // Echo character
      uart_send_char(ch);
      ui_question_handler(ch);
    } else if (waveform_active && ui_mode == 0) {
      // Waiting for F/A input
      uart_send_char(ch);
      ui_question_handler(ch);
    } else {
      // Normal command mode
...
static void cmd_freq(const char *params) {
  ...
  // Disable interrupts to safely update shared variable
  cli();
  freq = new_freq;
  sei();
...
static void cmd_run_adclab(const char *params) {
  ...
  aos_send("\r\n");
  aos_send("+-----------------------------------------------------------+\r\n");
  aos_send("|          PROGRAMMABLE WAVEFORM GENERATOR STATUS            |\r\n");
  aos_send("+-----------------------------------------------------------+\r\n");
