...
static void cmd_run_daclab(const char *params);
...
void ui_process_commands(void) {
  ...
  if (uart_receive_char(&ch)) {
    ...
    // Check if we're in DACLab mode using the DACLab module's state
    if (dac_lab_is_active()) {
      // In DACLab mode - let DACLab handle it (should not reach here)
      // because main loop calls dac_lab_process() instead
      return;
    } else {
...
static void cmd_run_daclab(const char *params) {
  (void)params;
  aos_send("\r\n");
  aos_send("Launching DACLab - Programmable Waveform Generator...\r\n");
  aos_send(".\r\n");
  _delay_ms(500);
  aos_send("..\r\n");
  _delay_ms(500);
  aos_send("...\r\n");
  _delay_ms(500);
  aos_send("AOS Has loaded the Subroutine: DACLab\r\n");

  aos_send("(Type EXIT to return to AOS)\r\n");
  
  // Initialize and launch DACLab
  dac_lab_init();
...
static void cmd_run_adclab(const char *params) {
  (void)params;
  aos_send("\r\n");
  aos_send("Launching ADCLab - Voltmeter...\r\n");
  aos_send(".\r\n");
  _delay_ms(500);
  aos_send("..\r\n");
  _delay_ms(500);
  aos_send("...\r\n");
  _delay_ms(500);
  aos_send("AOS Has loaded the Subroutine: ADCLab\r\n");

  aos_send("(Type EXIT to return to AOS)\r\n");
  
  // Initialize and launch ADCLab
  adc_lab_init();
  adc_lab_show_welcome();
}

static void cmd_run_eepromlab(const char *params) {
  (void)params;
  aos_send("\r\n");
  aos_send("Launching EEPROMLab - Password Manager...\r\n");
  aos_send(".\r\n");
  _delay_ms(500);
  aos_send("..\r\n");
  _delay_ms(500);
  aos_send("...\r\n");
  _delay_ms(500);
  aos_send("AOS Has loaded the Subroutine: EEPROMLab\r\n");

  aos_send("(Type E to exit)\r\n");
...
static void cmd_run_labtest3(const char *params) {
  (void)params;
  aos_send("\r\n");
  aos_send("Launching LabTest3 - LED & DAC Control...\r\n");
  aos_send(".\r\n");
  _delay_ms(500);
  aos_send("..\r\n");
  _delay_ms(500);
  aos_send("...\r\n");
  _delay_ms(500);
  aos_send("AOS Has loaded the Subroutine: LabTest3\r\n");
